...
import { users, markets, gameTypes } from './shared/schema';
  ...
  try {
    ...
    const [iplMarket] = await db
      ...
      .values({
        ...
        closingTime: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // 7 days from now
    ...
    const [worldCupMarket] = await db
      ...
      .values({
        ...
        status: 'open',
        openingTime: new Date(),
        closingTime: new Date(Date.now() + 10 * 24 * 60 * 60 * 1000), // 10 days from now
        createdBy: adminUser.id,
